...
const sortConfig = config => {
  ...
  if (config.address) config.address = ethers.ethers.utils.getAddress(config.address);
...
class SequenceUtilsFinder {
  constructor(authProvider) {
    ...
    this.findConfigForImageHash = async function (context, image, knownConfigs = [], skipCache = false) {
      ...
      utils.logger.info(`[findConfigForImageHash] image:${image}`);

      // Load index for last imageHash update
      const authContract = new ethers.Contract(context.sequenceUtils, abi.walletContracts.sequenceUtils.abi, _this.authProvider);
      const imageHashHeight = (await authContract.lastImageHashUpdate(image)).toNumber();
...
async function buildStubSignature(provider, config) {
  const multicallProvider = new multicall__namespace.providers.MulticallProvider(provider);
  ...
  let sortedSigners = signers.sort((a, b) => a.weight - b.weight);
  ...
  const stubSig = ethers.ethers.utils.arrayify("0x1fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff7fffffffffffffffffffffffffffffff5d576e7357a4501ddfe92f46681b20a01b02");
