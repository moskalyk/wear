...
class GuardRemoteSigner extends RemoteSigner {
  constructor(address, hostname, isSequence = false, defaultChainId = network.ChainId.MAINNET) {
    ...
    this._guard = new guard.Guard(hostname, fetch);
  ...
  async signMessageWithData(message, auxData, chainId) {
    ...
    const res = await this._guard.sign({
      request: request
    });
...
class Wallet extends Signer {
  ...
  useConfig(config, strict) {
    return new Wallet({
      ...
      strict
    }, ...this._signers).setProvider(this.provider, this.chainId).setRelayer(this.relayer);
  }
  useSigners(...signers) {
    return new Wallet({
      config: this.config,
      context: this.context
    }, ...signers).setProvider(this.provider, this.chainId).setRelayer(this.relayer);
  }
  ...
  connect(provider, relayer) {
    if (network.isJsonRpcProvider(provider)) {
      return new Wallet({
        config: this.config,
        context: this.context
      }, ...this._signers).setProvider(provider, this.chainId).setRelayer(relayer);
    } else {
  ...
  setProvider(provider, chainId) {
    ...
    if (ethers.providers.Provider.isProvider(provider)) {
      ...
      const jsonProvider = new network.JsonRpcProvider(provider, {
        chainId,
        blockCache: true
      });
  ...
  async sendTransaction(transaction, chainId, allSigners, quote, callback, waitForReceipt) {
  ...
  async sendTransactionBatch(transactions, chainId, allSigners = true, quote, callback, waitForReceipt) {
  ...
  async signTransactions(txs, chainId, allSigners) {
    ...
    let stx = await transactions.fromTransactionish(this.context, this.address, transaction);
    ...
    stx = transactions.appendNonce(stx, nonce);

    // Get transactions digest
    const digest = transactions.digestOfTransactionsNonce(nonce, ...stx);
  ...
  async sign(msg, isDigest = true, chainId, allSigners) {
    var _this = this;
    const signChainId = await this.getChainIdNumber(chainId);
    const digest = isDigest ? msg : ethers.ethers.utils.keccak256(msg);
    ...
    const remoteSignature = await signWith(remoteSigners, this.packMsgAndSig(digest, config.encodeSignature(localSignature), signChainId));
  ...
  async updateConfig(config, nonce, publish = false, indexed, quote, callback, waitForReceipt) {
    ...
    }, config), await this.sendTransaction(transactions.appendNonce(txs, n), undefined, undefined, quote, callback, waitForReceipt)];
  }
  ...
  async publishConfig(indexed, nonce, requireFreshSigners = [], quote, callback, waitForReceipt) {
    return this.sendTransaction(this.config.address ? this.buildPublishConfigTransaction(this.config, indexed, nonce) : await this.buildPublishSignersTransaction(indexed, nonce, requireFreshSigners), undefined, undefined, quote, callback, waitForReceipt);
  }
  ...
  async buildUpdateConfigTransaction(config$1, publish = false, indexed) {
    ...
    const mainModuleInterface = new ethers.utils.Interface(abi.walletContracts.mainModuleUpgradable.abi);
    const transaction = {
      delegateCall: false,
      revertOnError: true,
      gasLimit: ethers.ethers.constants.Zero,
    ...
    const transactions$1 = [...preTransaction, transaction, ...postTransaction];
  ...
  buildPublishConfigTransaction(config$1, indexed = true, nonce) {
    const sequenceUtilsInterface = new ethers.utils.Interface(abi.walletContracts.sequenceUtils.abi);
    return [{
      delegateCall: false,
      revertOnError: true,
      gasLimit: ethers.ethers.constants.Zero,
  ...
  async buildPublishSignersTransaction(indexed = true, nonce, requireFreshSigners = []) {
    ...
    const sequenceUtilsInterface = new ethers.utils.Interface(abi.walletContracts.sequenceUtils.abi);
    const requireFreshSignersInterface = new ethers.utils.Interface(abi.walletContracts.requireFreshSigner.abi);
...
class Account extends Signer {
  ...
  constructor(options, ...signers) {
    ...
    if (!options.context) this.options.context = _extends({}, network.sequenceContext);
  ...
  setNetworks(mainnetNetworks, testnetNetworks = [], defaultChainId) {
    ...
    this.options.networks = network.ensureValidNetworks(network.sortNetworks(networks));
...
async function isValidContractWalletSignature(address, digest, sig, provider) {
  ...
  try {
    ...
    const wallet = new ethers.ethers.Contract(address, abi.walletContracts.erc1271.abi, provider);
    const response = await wallet.isValidSignature(digest, sig);
    return abi.walletContracts.erc1271.returns.isValidSignatureBytes32 === response;
  } catch (_unused3) {
    return false;
  }
}
...
const fetchImageHash = async signer => {
  ...
  const walletContract = new ethers.Contract(address, abi.walletContracts.mainModuleUpgradable.abi, await signer.getProvider());
