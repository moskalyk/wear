...
class Session {
  ...
  static async open(args) {
    ...
    if (existingWallet) {
      ...
      if (!config$1) throw Error('Wallet config not found');
...
function getAuthProvider(networks) {
  ...
  return (_authChain$provider = authChain.provider) != null ? _authChain$provider : new network.JsonRpcProvider(authChain.rpcUrl, {
    chainId: authChain.chainId,
    blockCache: true
  });
}
...
const ValidateSequenceDeployedWalletProof = async (provider, chainId, proof) => {
  ...
  const contract = new ethers.ethers.Contract(proof.address, abi, provider);
