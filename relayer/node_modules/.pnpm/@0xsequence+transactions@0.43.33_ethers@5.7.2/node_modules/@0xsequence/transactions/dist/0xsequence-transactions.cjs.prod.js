...
function digestOfTransactionsNonce(nonce, ...txs) {
  return ethers.ethers.utils.keccak256(packMetaTransactionsNonceData(nonce, ...txs));
}
...
async function toSequenceTransactions(wallet, txs, revertOnError = false, gasLimit) {
...
async function toSequenceTransaction(wallet, tx, revertOnError = false, gasLimit, nonce) {
...
function sequenceTxAbiEncode(txs) {
  return txs.map(t => {
    ...
    return {
      ...
      target: (_t$to = t.to) != null ? _t$to : ethers.ethers.constants.AddressZero,
...
async function fromTransactionish(context, wallet, transaction) {
  ...
  if (Array.isArray(transaction)) {
    if (hasSequenceTransactions(transaction)) {
      stx = flattenAuxTransactions(transaction);
    } else {
...
exports.digestOfTransactionsNonce = digestOfTransactionsNonce;
exports.encodeNonce = encodeNonce;
exports.flattenAuxTransactions = flattenAuxTransactions;
exports.fromTransactionish = fromTransactionish;
...
exports.packMetaTransactionsNonceData = packMetaTransactionsNonceData;
